...
public class FR94Parser implements DatasetParser<FR94Data> {
    ...
    @Override
    public void parse(File fr94Directory, DocumentConsumer<FR94Data> consumer) throws IOException {
        traverseDirectory(fr94Directory, consumer);
    }

    private void traverseDirectory(File directory, DocumentConsumer<FR94Data> consumer) throws IOException {
    ...
    public void parseSingleFile(File fr94File, DocumentConsumer<FR94Data> consumer) throws IOException {
        if (fr94File.isFile()) {
    ...
    private void parseFR94File(File fr94File, DocumentConsumer<FR94Data> consumer) throws IOException {
        String fileContent;
        try {
        ...
        for (Element element : doc.select("DOC")) {
            ...
            element.select("ADDRESS, SIGNER, SIGNJOB, BILLING, FRFILING, DATE, RINDOCK").remove();

            // Extracting required fields
            fr94Data.setDocNo(element.select("DOCNO").text());
            fr94Data.setDocTitle(element.select("DOCTITLE").text());

            String agency = element.select("AGENCY").text();
            if (agency.startsWith("AGENCY:")) {
                agency = agency.substring("AGENCY:".length()).trim();
            }
            fr94Data.setAgency(agency);
            fr94Data.setUsDept(element.select("USDEPT").text());

            String summary = element.select("SUMMARY").text();
            if (summary.startsWith("SUMMARY:")) {
                summary = summary.substring("SUMMARY:".length()).trim();
            }
            fr94Data.setSummary(summary);

            String uncleanedText = element.select("TEXT").text();
            String cleanedText = cleanText(uncleanedText);
            fr94Data.setFullText(cleanedText);
    ...
    private String cleanText(String content) {
        content = content.replaceAll("<AGENCY>.*?</AGENCY>", "");
        content = content.replaceAll("<DOCTITLE>.*?</DOCTITLE>", "");
        content = content.replaceAll("<USDEPT>.*?</USDEPT>", "");
        content = content.replaceAll("<SUMMARY>.*?</SUMMARY>", "");
        content = content.replaceAll("<IMPORT>.*?</IMPORT>", "");

        content = content.replaceAll("<!--.*?-->", "");
        content = content.replaceAll("<(ITAG|QTAG|FTAG|STAG|TTAG|ZTAG)[^>]*>", "");
